...
|费用|不要钱|高昂的费用|https协议需要到ca申请证书，一般免费证书较少，因而需要一定费用
...
#### 容器
1. flex-direction，决定主轴的方向。 默认值为row。 取值：row|col|row-reverse|col-reverse
2. flex-wrap，如果一条轴线排不下，如何换行。 可设置的值为wrap、nowrap, 默认值为nowrap
3. flex-flow，flex-direction属性和flex-wrap属性的简写形式
4. justify-content，项目在主轴上的对齐方式，具体对齐方式与轴的方向有关。取值：flex-start, flex-end, center, space-around, space-between
5. align-items，项目在交叉轴上如何对齐。取值：flex-start， flex-end， center
6. align-content
